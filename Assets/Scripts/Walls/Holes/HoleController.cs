...
public class HoleController : MonoBehaviour
{
    ...
    void AdjustTargetOffset()
    {
        ...
        if (targetAngle != prevTargetAngle)
        {
            ...
            if (useOptimals)
            {
                if (lowerOptimalAngle < upperOptimalAngle)
                { // checking if the lower optimal angle is actually lower, not upper
                    if (targetAngle > lowerOptimalAngle && targetAngle < upperOptimalAngle)
                    { // checking if the target angle is withing optimals
                        angleWithin = true;
                    }
                    else {
                        angleWithin = false;
                    ...
                    if (targetAngle > upperOptimalAngle && targetAngle < lowerOptimalAngle)
